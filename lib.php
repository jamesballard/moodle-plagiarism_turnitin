...
class plagiarism_plugin_turnitin extends plagiarism_plugin {
    ...
    /**
     * Prints the disclosure for a single student
     * 
    ...
    public function get_links($linkarray) {
        ...
        $modname = $linkarray['modname'];
        if (isset($linkarray[$modname])) {
            $module = $linkarray[$modname];
        } else {
            $sql = "SELECT a.* FROM {".$modname."} a, {course_modules} cm WHERE cm.id= ? AND cm.instance = a.id";
        ...
        if (($USER->id == $userid) || has_capability('moodle/plagiarism_turnitin:viewsimilarityscore', $modulecontext)) {
            if ($plagiarismsettings = $this->get_settings()) {
                $plagiarismfile = $DB->get_record_sql(
                    ...
                    $DB->sql_compare_text('identifier')." = ?", array($cmid, $userid, $linkarray['file']->get_contenthash()));
    ...
    /**
     * Saves the turnitin settings from the module form
     * 
     * @global moodle_database $DB
     * @param object $data
     * @return void 
...
//        error_log('save form fired'); // ULCC debug
	    global $DB, $USER;
        ...
        // ULCC hack - triggers the create event a second time (it's instant) after the plagiarism 
        // stuff has been saved. The first time fails because it can't find a setting saying to use
        // turnitin. We don't need to worry about create/edit as the plugin uses the same function 
        // to handle both
    ...
    /**
     * Adds config elements to a module form, so that turnitin settings can be configured for that
     * module instance.
     * 
     * @global type $CFG
     * @global type $DB
    ...
    /**
     * Outputs the disclosure of similarity for a module
     * 
     * @global type $DB
     * @global type $OUTPUT
     * @param type $cmid 
    ...
    public function get_settings() {
        ...
        if (isset($plagiarismsettings['turnitin_use']) && $plagiarismsettings['turnitin_use'] && isset($plagiarismsettings['turnitin_accountid']) && $plagiarismsettings['turnitin_accountid']) {
            ...
            if (empty($plagiarismsettings['turnitin_secretkey'])) {
                print_error("Turnitin Secret Key not set!");
            }
    ...
    /**
     * Lists the options that should be present in the module form
     * 
     * @return array 
     */
    public function config_options() {
        return array(
                ...
                'plagiarism_anonymity');
    }

    /**
     * Gets the status of the assignment from turnitin, including submitted files and their similarity scores,
     * then updates the turnitin tables
     * 
     * @global moodle_database $DB
     * @global type $USER
     * @param type $course
    ...
    public function update_status($course, $cm) {
        ...
        // $tii['diagnostic'] = '1'; // uncomment for debugging. Only useful for viewing html output - no xml from this.
        $url = turnitin_get_url($tii, $plagiarismsettings);
        $tiixml = plagiarism_get_xml($url);
        if ($tiixml) {
    ...
    public function cron() {
        ...
        if (!empty($plagiarismsettings['turnitin_attempts']) && is_numeric($plagiarismsettings['turnitin_attempts'])
            ...
            foreach ($items as $item) {
                ...
                $cm = get_coursemodule_from_id('', $item->cm);
                $module = $DB->get_record_sql("SELECT m.name FROM {$CFG->prefix}modules m LEFT JOIN {$CFG->prefix}course_modules cm ON m.id = cm.module WHERE cm.id = $item->cm");
                $fs = get_file_storage();
                ...
                } elseif ($module->name == 'coursework') {
                    require_once($CFG->dirroot.'/mod/coursework/locallib.php');
                    $coursework = new coursework($cm->instance);
                    $submission = $DB->get_record('coursework_submissions', array('courseworkid' => $cm->instance, 'userid' => $item->userid));
                    $submission = new coursework_submission($submission, $coursework);
                }
                if ($files = $fs->get_area_files($modulecontext->id, 'mod_'.$module->name, 'submission', $submission->id)) {
    ...
    /**
     * Decides what to do with event data from various events
     * 
     * @global moodle_database $DB
     * @global stdClass $CFG
    ...
    public function event_handler($eventdata) {
        ...
        $result = true;
        $supportedmodules = array('assignment', 'coursework', 'quiz');
        ...
        $plagiarismvalues = $DB->get_records_menu('turnitin_config', array('cm' => $cmid), '', 'name,value');
        // has turnitin_use on create event, use_turnitin afterwards
        if (!$plagiarismsettings || (empty($plagiarismvalues['use_turnitin']) && empty($plagiarismvalues['turnitin_use']))) {
            //nothing to do here... move along!
            return $result;
        }
        ...
        if ($eventdata->eventtype == "mod_created") {
            ...
            if (!empty($eventdata->files)) { //this is an upload event with multiple files
                ...
                if (isset($plagiarismvalues['plagiarism_draft_submit']) && $plagiarismvalues['plagiarism_draft_submit'] == 1) { // is file to be sent on final submission?
                    ...
                    $fs = get_file_storage();
                    if ($files = $fs->get_area_files($modulecontext->id, 'mod_'.$eventdata->modulename, 'submission', $submission->id, "timemodified")) {
                        foreach ($files as $file) {
                            if ($file->get_filename() === '.') {
                                continue;
                            }
...
function turnitin_update_assignment($plagiarismsettings, $plagiarismvalues, $eventdata, $action) {
    ...
    if ($action == 'create' or $action == 'update') { //TODO: split this into 2 - we don't need to call the create if we know it already exists.
        ...
        //first check if this assignment has already been created
            ...
            // Make the assignment anonymous if required. Defaults to non-anonymous
            if (isset($plagiarismvalues['plagiarism_anonymity'])) {
                error_log('setting anonymous turnitin option');
                $tii['anon'] = $plagiarismvalues['plagiarism_anonymity'] ? 1 : 0;
            } else {
                error_log('NOT setting anonymous turnitin option');
            }
...
function plagiarism_get_xml($url) {
    global $CFG;
    require_once($CFG->libdir."/filelib.php");
    if (!($fp = download_file_content($url, null, null, null, null, null, true))) { // FIX ME! should just be ($url) in production
        print_error("error trying to open plagiarism XML file!".$url);
    } else {
...
function plagiarism_update_record($cmid, $userid, $identifier, $attempt=0) {
    ...
    $plagiarism_file = $DB->get_record_sql(
        "SELECT * FROM {turnitin_files}
                                 WHERE cm = ? AND userid = ? AND ".
        $DB->sql_compare_text('identifier')." = ?", array($cmid, $userid, $identifier));
    if (!empty($plagiarism_file)) {
        ...
        if ($plagiarism_file->statuscode !== 'pending' or
            $plagiarism_file->statuscode !== 'success' or
